<pattern xmlns="http://purl.oclc.org/dsdl/schematron" abstract="true" id="Step.01">
	<rule context="$Model">
	    <assert test="$EIRA-001" flag="fatal" id="EIRA-001">[EIRA-001] The model must define a view named 'Legal view'.</assert>
	    <assert test="$EIRA-002" flag="fatal" id="EIRA-002">[EIRA-002] The model must define a view named 'Organisational view'.</assert>
	    <assert test="$EIRA-003" flag="fatal" id="EIRA-003">[EIRA-003] The model must define a view named 'Semantic view'.</assert>
	    <assert test="$EIRA-004" flag="fatal" id="EIRA-004">[EIRA-004] The model must define a view named 'Technical view - application'.</assert>
	    <assert test="$EIRA-005" flag="fatal" id="EIRA-005">[EIRA-005] The model must define a view named 'Technical view - infrastructure'.</assert>
	    <assert test="$EIRA-006" flag="fatal" id="EIRA-006">[EIRA-006] The model must define a view named 'Highlevel viewpoint'.</assert>
