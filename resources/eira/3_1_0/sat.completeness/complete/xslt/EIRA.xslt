...
<!--SCHEMA SETUP-->
<xsl:template match="/">
    <svrl:schematron-output schemaVersion="" title="EIRA v3.1.0 - SAT Completeness Profile - Complete level">
      <xsl:comment>
...
<!--SCHEMATRON PATTERNS-->
<svrl:text>EIRA v3.1.0 - SAT Completeness Profile - Complete level</svrl:text>
...
<xsl:choose>
      ...
      <xsl:otherwise>
		...
		<!--ASSERT -->
<xsl:choose>
      <xsl:when test="exists(a:elements/a:element[matches(substring-after(normalize-space(substring-before(normalize-space(substring-after(string(a:name), '&lt;&lt;')), '>>')), 'eira:'), 'Service Registration Service')])" />
      <xsl:otherwise>
        <svrl:failed-assert test="exists(a:elements/a:element[matches(substring-after(normalize-space(substring-before(normalize-space(substring-after(string(a:name), '&lt;&lt;')), '>>')), 'eira:'), 'Service Registration Service')])">
          <xsl:attribute name="id">EIRA-024-05</xsl:attribute>
          <xsl:attribute name="flag">warning</xsl:attribute>
          <xsl:attribute name="location">
            <xsl:apply-templates mode="schematron-select-full-path" select="." />
          </xsl:attribute>
          <svrl:text>[EIRA-024] SBBs should be defined for all key interoperability enablers. No 'Service Registration Service' SBB is defined.</svrl:text>
        </svrl:failed-assert>
      </xsl:otherwise>
    </xsl:choose>
