...
<xsl:stylesheet version="1.0"
    ...
    <xsl:variable name="eiraDoc" select="document('EIRA.xml')"/>
    ...
    <xsl:function name="eira:abbFromStereotypedName" as="xs:string">
        ...
        <xsl:value-of select="substring-after(normalize-space(substring-before(normalize-space(substring-after(string($stereotypedName), '&lt;&lt;')), '&gt;&gt;')), 'eira:')"/>
    </xsl:function>

    <xsl:function name="eira:sbbFromStereotypedName" as="xs:string">
        <xsl:param name="stereotypedName"/>
        <xsl:value-of select="normalize-space(substring-after(string($stereotypedName), '&gt;&gt;'))"/>
    </xsl:function>
    ...
    <xsl:function name="eira:isAbbToIgnore" as="xs:boolean">
        ...
        <xsl:value-of select="
            ...
            matches($abbName, '(Interoperable European Solution|Interoperable European Solution Component|Architecture Building Block|EIF Interoperability Level|EIRA Architecture Building Block|EIRA Solution Building Block|EIRA View|EIRA Viewpoint|Key Interoperability Enabler|Public Service Component|Public Service Manifestation|Solution|Solution Building Block)')
        "/>
    </xsl:function>
    ...
    <xsl:function name="eira:isLeafAbbInEira" as="xs:boolean">
        ...
        <xsl:value-of select="let $name := string($element/a:name) return exists($eiraDoc/a:model/a:elements/a:element[a:properties/a:property[@propertyDefinitionRef = string($eiraDoc/a:model/a:propertyDefinitions/a:propertyDefinition[a:name = 'dct:type']/@identifier)] and string(a:name) = $name and (let $elementId := string(@identifier) return not(exists($eiraDoc/a:model/a:relationships/a:relationship[string(@target) = $elementId and @xsi:type = 'Specialization'])))])"/>
    </xsl:function>
            ...
            <pattern xmlns="http://purl.oclc.org/dsdl/schematron" id="Step.01">
                <rule context="/a:model">
                    ...
                    <xsl:for-each select="/a:model/a:elements/a:element[eira:isSbb(a:name) and not(eira:isSbbToIgnore(a:name)) and not(eira:isSpecification(.))]">
                        ...
                        <assert id="{$ruleId}" test="exists(a:elements/a:element[let $abbName := normalize-space(substring-after(normalize-space(substring-before(normalize-space(substring-after(string(a:name), '&lt;&lt;')), '&gt;&gt;')), 'eira:')) return (let $sbbName := normalize-space(substring-after(normalize-space(string(a:name)), '&gt;&gt;')) return ($sbbName = '{eira:sanitizeForQuery($sbbName)}' and ({eira:tokenStringToOr($acceptedAbbs, '|', '$abbName')})))])">[<xsl:value-of select="$ruleId"/>] [<xsl:value-of select="$satName"/>] SBB '<xsl:value-of select="eira:sbbFromStereotypedName(current()/a:name)"/>' for ABB '<xsl:value-of select="eira:abbFromStereotypedName(current()/a:name)"/>' is expected and must be included in the solution.</assert>
<!--
exists(
...
-->                        
                    ...
                    <xsl:for-each select="/a:model/a:elements/a:element[eira:isAbb(a:name) and not(eira:isAbbToIgnore(a:name)) and not(eira:isSpecialisedByOtherAbb(.))]">
                        ...
                        <assert id="{$ruleId}" test="exists(a:elements/a:element[let $abbName := normalize-space(substring-after(normalize-space(substring-before(normalize-space(substring-after(string(a:name), '&lt;&lt;')), '&gt;&gt;')), 'eira:')) return ({eira:tokenStringToOr($acceptedAbbs, '|', '$abbName')})])"><xsl:value-of select="$assertionMessage"/></assert>
<!--
exists(
    a:elements/a:element[
        let $abbName := normalize-space(substring-after(normalize-space(substring-before(normalize-space(substring-after(string(a:name), '&lt;&lt;')), '&gt;&gt;')), 'eira:')) return (
...
-->                                
                    ...
                    <xsl:for-each select="/a:model/a:elements/a:element[eira:isSpecification(.)]">
                        ...
                        <xsl:for-each select="$abbsLinkedToSpecification">
                            ...
                            <assert id="{$ruleId}" test="exists(a:elements/a:element[let $elementIdentifier := @identifier return (let $declaredAbbName := substring-after(normalize-space(substring-before(normalize-space(substring-after(string(a:name), '&lt;&lt;')), '&gt;&gt;')), 'eira:') return ({eira:tokenStringToOr($allAcceptedAbbs, '|', '$declaredAbbName')}) and exists(/a:model/a:relationships/a:relationship[@xsi:type = 'Association' and (@target = $elementIdentifier or @source = $elementIdentifier) and (let $associatedIdentifier := (if (@target = $elementIdentifier) then @source else @target) return (exists(/a:model/a:elements/a:element[@identifier = $associatedIdentifier and (let $specAbbName := normalize-space(substring-after(normalize-space(substring-before(normalize-space(substring-after(string(a:name), '&lt;&lt;')), '&gt;&gt;')), 'eira:')) return (let $specSbbName := normalize-space(substring-after(normalize-space(string(a:name)), '&gt;&gt;')) return ($specSbbName = '{eira:sanitizeForQuery($specSbbName)}' and ({eira:tokenStringToOr($allAcceptedSpecs, '|', '$specAbbName')}))))])))]))])"><xsl:value-of select="$assertionMessage"/></assert>
<!--
    exists(
        a:elements/a:element[
            let $elementIdentifier := @identifier return (
...
-->                             
                        ...
                        <xsl:if test="not(exists($abbsLinkedToSpecification)) and not(exists($sbbsLinkedToSpecification))">
                            ...
                            <assert id="{$ruleId}" test="exists(a:elements/a:element[let $abbName := normalize-space(substring-after(normalize-space(substring-before(normalize-space(substring-after(string(a:name), '&lt;&lt;')), '&gt;&gt;')), 'eira:')) return (let $sbbName := normalize-space(substring-after(normalize-space(string(a:name)), '&gt;&gt;')) return ($sbbName = '{eira:sanitizeForQuery($sbbName)}' and ({eira:tokenStringToOr($acceptedAbbs, '|', '$abbName')})))])">[<xsl:value-of select="$ruleId"/>] [<xsl:value-of select="$satName"/>] Specification '<xsl:value-of select="eira:sbbFromStereotypedName(current()/a:name)"/>' of type '<xsl:value-of select="eira:abbFromStereotypedName(current()/a:name)"/>' is expected and must be included in the solution.</assert>
                        </xsl:if>
                    </xsl:for-each>
                </rule>
            </pattern>
