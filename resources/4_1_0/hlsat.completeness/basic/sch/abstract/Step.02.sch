<pattern xmlns="http://purl.oclc.org/dsdl/schematron" abstract="true" id="Step.02">
	...
	<rule context="$SBBsForIOPPrincipleViewABBs">
		<assert test="$EIRA-016" flag="warning" id="EIRA-016">[EIRA-016] SBB '<value-of select="$SBBName"/>' (<value-of select="$SBBDeclaredABBName"/>) is not present in the model's Architectural Principles view.</assert>
	</rule>
	<rule context="$SBBsForTechViewABBs">
 	...
 	<rule context="$ABBsForIOPPrincipleView">
	    <assert test="$EIRA-034" flag="warning" id="EIRA-034">[EIRA-034] ABB '<value-of select="$ABBName"/>' is not present in the model's Architectural Principles view.</assert>
 	</rule>
