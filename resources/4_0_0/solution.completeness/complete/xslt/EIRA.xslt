...
<!--SCHEMA SETUP-->
<xsl:template match="/">
    <svrl:schematron-output schemaVersion="" title="EIRA v4.0.0 - Solution Completeness Profile - Complete level">
      <xsl:comment>
...
<!--SCHEMATRON PATTERNS-->
<svrl:text>EIRA v4.0.0 - Solution Completeness Profile - Complete level</svrl:text>
...
<xsl:choose>
      ...
      <xsl:otherwise>
		...
		<!--ASSERT -->
<xsl:choose>
      <xsl:when test="exists(a:elements/a:element[matches(normalize-space(substring-after(normalize-space(substring-before(normalize-space(substring-after(string(a:name), '&lt;&lt;')), '>>')), 'eira:')), 'Interoperable Digital Public Service')])" />
      <xsl:otherwise>
        <svrl:failed-assert test="exists(a:elements/a:element[matches(normalize-space(substring-after(normalize-space(substring-before(normalize-space(substring-after(string(a:name), '&lt;&lt;')), '>>')), 'eira:')), 'Interoperable Digital Public Service')])">
          <xsl:attribute name="id">EIRA-018-04</xsl:attribute>
          <xsl:attribute name="flag">fatal</xsl:attribute>
          <xsl:attribute name="location">
            <xsl:apply-templates mode="schematron-select-full-path" select="." />
          </xsl:attribute>
          <svrl:text>[EIRA-018] All ABBs in the high-level overview must be defined. No 'Interoperable Digital Public Service' SBB is defined.</svrl:text>
        </svrl:failed-assert>
      </xsl:otherwise>
    </xsl:choose>
